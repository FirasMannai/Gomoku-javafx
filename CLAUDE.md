...
- `GomokuGameView` sidebar (260 px) holds: a prominent **turn badge** (`setTurn`, tinted on Red's turn via the `:red-turn` pseudo-class), a side-by-side **score card**, an info card (move count + elapsed clock + last move + AI/your time), a scrollable **move-history** `ListView` (`addMove`/`clearHistory`), and Save/Load. The toolbar carries the action buttons + a theme `ComboBox`.
